...
const WriteProfileToCloudFirestore = (postData) => {
  const { user } = useUser()
  const {user_name, belong, profile} = postData;
  const sendData = () => {
    try {
      firebase
        .firestore()
...
export default WriteProfileToCloudFirestore